...
SFT_FROM_RL_PATHS = [
    f"/cephfs/zhanghuaqing/RL/LLaMA-Factory/saves/Llama3B_gsm8k_SFT_rltrained_300_47rollout/checkpoint-{ckpt}"
    for ckpt in range(480, 4801, 480)
]
RL_PATHS=[
    f"/cephfs/zhanghuaqing/RL/rllm_deepscaler/checkpoints/deepscaler/Llama3_GRPO_run2/actor/global_step_{ckpt}" 
    for ckpt in range(20, 301, 40) 
]
MODEL_PATHS = RL_PATHS + SFT_FROM_RL_PATHS + [
...
for path in MODEL_PATHS:
    ...
    if not os.path.exists(file_path):
        file_path = os.path.join(path, "boxed_test_result_T0p6_S8__shards", "batch_000000.parquet")
    if not os.path.exists(file_path):
...
plt.grid(True)
plt.tight_layout()
plt.savefig("experiments/3.2_300step/model_accuracy_comparison.pdf")
